...
RSpec.describe AuthorizationModule::PolicyEnforcer, type: :unit do
  subject(:policy_enforcer) do
    described_class.new(attribute_provider_factory: AuthorizationModule::Abac::AttributeProviderFactory.new)
  end
  ...
  describe '#action_allowed?(action, resource_type, user, context)' do
    ...
    context 'when user has a permission that denies the action' do
      before do
        permissions = [
            instance_double('Identity::Permission', allows_on_resource?: true, denies?: false, denies_on_resource?: false),
            instance_double('Identity::Permission', denies?: true, allows_on_resource?: false, denies_on_resource?: true)
        ]
    ...
    context 'when user has a permission that allows the action' do
      before do
        permissions = [
            instance_double('Identity::Permission', allows_on_resource?: true, denies_on_resource?: false)
        ]
        allow(user).to receive(:hierarchical_permissions) { [permissions] }
        allow(user).to receive(:permissions) { permissions }
      end
    ...
    context 'when user has no permission that either allows or denies the action' do
      before do
        permissions = [
            instance_double('Identity::Permission', allows_on_resource?: false, denies_on_resource?: false),
            instance_double('Identity::Permission', denies_on_resource?: false, allows_on_resource?: false)
        ]
        allow(user).to receive(:hierarchical_permissions) { [permissions] }
        allow(user).to receive(:permissions) { permissions }
      end
